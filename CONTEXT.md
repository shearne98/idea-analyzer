...
Idea Analyzer owns readiness as a `readiness.json`-compatible contract over normalized idea content only. Readiness accepts normalized markdown or a parsed normalized idea object, returns blockers, warnings, and suggestions, and defines final-analysis readiness as zero blockers. It must not inspect `source.md`, Founder Profile, or Hearne OS workspace paths.
